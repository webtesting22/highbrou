...
const ApplicationView = () => {
  ...
  const fetchApplications = async () => {
    ...
    try {
      const response = await fetch(
        `${apibaseUrl}/highbrou/getAllJobApplications`,
      );
  ...
  const filteredApplications = applications
    .filter(
      (app) =>
        app.currentCTC >= currentCTCRange[0] &&
        app.currentCTC <= currentCTCRange[1],
    )
    .filter(
      (app) =>
        app.expectedCTC >= expectedCTCRange[0] &&
        app.expectedCTC <= expectedCTCRange[1],
    )
    ...
    .filter((app) => {
      if (selectedCTCRanges.length > 0) {
        return selectedCTCRanges.some(
          ([min, max]) => app.currentCTC >= min && app.currentCTC <= max,
        );
      }
      return true;
    });
  ...
  const qualificationFilters = [
    ...new Set(
      applications.map((app) => app.highestQualification).filter(Boolean),
    ),
  ...
  const handleClearAllFilters = () => {
    ...
    setCurrentCTCRange([
      Math.min(...currentCTCValues),
      Math.max(...currentCTCValues),
    ]);
    setExpectedCTCRange([
      Math.min(...expectedCTCValues),
      Math.max(...expectedCTCValues),
    ]);
  };
      ...
      <section id="ApplicationViewContainer" className="sectionPadding">
        ...
        >
          ...
          >
            {[
              ...new Set(
                applications.map((app) => app.jobPosition).filter(Boolean),
              ),
            ].map((pos) => (
          ...
          >
            {[
              ...new Set(
                applications.map((app) => app.passingYear).filter(Boolean),
              ),
            ]
              .sort()
              .map((year) => (
          ...
          <div style={{ width: 300 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <InputNumber
                ...
                onChange={(value) =>
                  setCurrentCTCRange([value, currentCTCRange[1]])
                }
              />
              <span>to</span>
              <InputNumber
                ...
                onChange={(value) =>
                  setCurrentCTCRange([currentCTCRange[0], value])
                }
              />
            </div>
            <Slider
          ...
          <div style={{ width: 300 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <InputNumber
                ...
                onChange={(value) =>
                  setExpectedCTCRange([value, expectedCTCRange[1]])
                }
              />
              <span>to</span>
              <InputNumber
                ...
                onChange={(value) =>
                  setExpectedCTCRange([expectedCTCRange[0], value])
                }
              />
            </div>
            <Slider
        ...
        <div style={{ overflow: "auto", width: "100%" }}>
          <Table
            columns={columns}
            dataSource={filteredApplications.map((app) => ({
              ...app,
              key: app._id,
            }))}
            loading={loading}
        ...
        <button
          onClick={handleDownloadCsvForGoogleSheets}
          style={{ padding: "0 12px", height: "32px" }}
        >
